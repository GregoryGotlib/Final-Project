...
#define LOG_INFO_BROADCAST(x)	Logger::getInstance()->info_everywhere(x)
#define LOG_ERROR_BROADCAST(x)  Logger::getInstance()->error_everywhere(x)
	...
	{
	public:
		static void setLogDirectory(std::string const &logDir);
		...
		void error_everywhere(const char* text) throw();
		void error_everywhere(std::string& text) throw();
		void error_everywhere(std::ostringstream& stream) throw();
		...
		void info_everywhere(const char* text) throw();
		void info_everywhere(std::string& text) throw();
		void info_everywhere(std::ostringstream& stream) throw();
