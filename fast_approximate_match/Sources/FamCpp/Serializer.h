...
class Serializer
{
	...
	public:
		...
		static void serializePcnMap(unordered_map<QuerySeed, unordered_map<int, vector<PcnEntry>>>& map, string const &outDirPath, int level);
		static void deserializePcnMap(string path, unordered_map<QuerySeed, unordered_map<int, vector<PcnEntry>>>* map, int* level);
};
