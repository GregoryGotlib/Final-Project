...
void Serializer::serializePcnMap(unordered_map<QuerySeed, unordered_map<int, vector<PcnEntry>>>& map, string const & outDirPath, int level)
{
	...
	LOG_INFO_BROADCAST("Serialization to " + path + ". Level " + to_string(level));
...
void Serializer::deserializePcnMap(string path, unordered_map<QuerySeed, unordered_map<int, vector<PcnEntry>>>* map, int* level)
{
	LOG_INFO_BROADCAST("Deserialization from " + path);
