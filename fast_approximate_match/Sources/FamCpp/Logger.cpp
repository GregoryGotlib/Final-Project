...
string createLogFileName()
{
	...
	sprintf(fileName, "Log %02d-%02d-%d %02d-%02d-%02d.log",
...
void Logger::setLogDirectory(string const &logDir)
{
	if (!experimental::filesystem::exists(logDir.c_str()))
	{
		_mkdir(logDir.c_str());
	}

	logFileName = logDir + "/" + createLogFileName();
}
...
string Logger::getCurrentTime()
{
	...
	char time[100];
	timeinfo = localtime(&rawtime);
	sprintf(time, "%02d/%02d/%d %02d:%02d:%02d",
...
void Logger::error_everywhere(const char* text) throw()
{
	string data;
	data.append("[ERROR]: ");
	data.append(text);
...
void Logger::error_everywhere(std::string& text) throw()
{
	error_everywhere(text.data());
}

void Logger::error_everywhere(std::ostringstream& stream) throw()
{
	string text = stream.str();
	error_everywhere(text.data());
}
...
void Logger::info_everywhere(const char* text) throw()
{
	string data;
	data.append("[INFO]: ");
	data.append(text);
...
void Logger::info_everywhere(std::string& text) throw()
{
	info_everywhere(text.data());
}

void Logger::info_everywhere(std::ostringstream& stream) throw()
{
	string text = stream.str();
	info_everywhere(text.data());
}
