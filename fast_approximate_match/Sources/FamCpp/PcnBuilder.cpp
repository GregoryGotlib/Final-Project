...
PcnBuilder::PcnBuilder(list<McsData> *mcsListPtr, unordered_map<string, vector<MappedEntry>> *allTextMapPtr, ConfigurationData const &confData)
{
	mcsList = mcsListPtr;
...
void::PcnBuilder::appendPcnEntries(const QuerySeed* querySeedPtr, unordered_map<int, vector<PcnEntry>> *pcnPtr, vector<SimilarityCandidate> *subPcn, int curLevel)
{
	bool isExists = false;
	bool duplicatedSeedContent = false;
	int lastLevel = pcnPtr->size();
	...
	for (int i = 0; i < subPcn->size(); i++)
	{
		...
		for (int j = 0; j < curLvlPcn.size(); j++)
		{
		...
		for (const auto& pcnLevel : *pcnPtr)
		{
			...
			for (int j = 0; j < pcnLevel.second.size(); j++)
			{
				if (pcnLevel.second[j].offsetInSeq == (*subPcn)[i].entry.offset &&
					Helpers::strCmp(pcnLevel.second[j].sequenceName.c_str(), (*subPcn)[i].entry.seqPtr->seqName.c_str()))
				{
		...
		if (!isExists)
		{
			...
			p.seedContent = Helpers::substringOfCString((*subPcn)[i].entry.seqPtr->seqData.c_str(), p.offsetInSeq, sizeOfSeed);
...
bool PcnBuilder::checkComplexity(const MappedEntry & candidate, int length)
{
	...
	for (int i = 0; i < alphabet.length(); i++)
	{
		...
		{
			if (candidate.seqPtr->seqData[candidate.indexInSeq + j] == alphabet[i])
			{
				count++;
			}
		}
		...
		if (count / length > complexityThreshold)
		{
			LOG_INFO_BROADCAST("Candidate: " + candidate.seqPtr->seqData.substr(candidate.indexInSeq, length) 
				+ " rejected with complexity threshold " + std::to_string(count/length) + " by letter " + alphabet[i] +
				+ ". Sequence: " + candidate.seqPtr->seqName + ". Index: " + std::to_string(candidate.indexInSeq) + "." );
			return false;
		}
	}
	return true;
...
void PcnBuilder::findSimilarToSeed(const char* seedPtr, int seedIndex, int bucket, vector<SimilarityCandidate>* candidates)
{
	int frameSize = 0;
	...
	for (list<McsData>::iterator currentMcs = (*mcsList).begin(); currentMcs != (*mcsList).end(); ++currentMcs)
	{
		...
		while (seedIsAlive)
		{
			...
			if ((*currentMcs).missIndexes.size() > 0)
			{
				...
				for (std::list<int>::iterator it = (*currentMcs).missIndexes.begin(); it != (*currentMcs).missIndexes.end(); ++it) {
					seedCurrentFrame[*it] = '.';
				}
			}

			seedIsAlive = (currentFrameIndex + 1 + frameSize) <= strlen(seedPtr);
				...
				candidate != allTextMap[bucket][seedCurrentFrame].end(); ++candidate)
			{
				...
				int hammDistance = Helpers::getHammingDistance(seedPtr, 0, strlen(seedPtr), candidate->seqPtr->seqData.c_str(), candidateStartOffset, length);
